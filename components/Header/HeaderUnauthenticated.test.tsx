...
describe("HeaderUnauthenticated", () => {
    it("should render without any modal open", () => {
        ...
        expect(screen.queryByText(labels.component.LoginForm.WELCOME_TO_PINIT)).toBeNull();
        expect(screen.queryByText(labels.component.SignupForm.FIND_NEW_IDEAS)).toBeNull();
    });
    ...
    it("should open login modal when clicking on Login button, and switch to signup modal when user clicks on 'Sign up'", async () => {
        ...
        const noAccountYetDiv = screen.getByText(labels.component.LoginForm.NO_ACCOUNT_YET);
        const noAccountYetSignupButton = within(noAccountYetDiv).getByText(labels.component.LoginForm.SIGN_UP);
    ...
    it("should open signup modal when clicking on Signup button, and switch to login modal when user clicks on 'Log in'", async () => {
        ...
        const alreadyHaveAccountDiv = screen.getByText(labels.component.SignupForm.ALREADY_HAVE_ACCOUNT);
        const alreadyHaveAccountSignupButton = within(alreadyHaveAccountDiv).getByText(labels.component.SignupForm.LOG_IN);
